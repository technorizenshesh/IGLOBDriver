...
//http://technorizen.com/WORKSPACE1/shipper/webservice/get_user?user_id=61
    ...
    private class ValidCard extends AsyncTask<String, String, String> {
        @Override
        protected void onPreExecute() {
            super.onPreExecute();
            // prgressbar.setVisibility(View.VISIBLE);
        ...
        @Override
        protected String doInBackground(String... strings) {
//http://hitchride.net/webservice/valid_card?token=xyz
            try {
                Log.e("token_str ", " >>" + token_str);
                String postReceiverUrl = BaseUrl.baseurl + "valid_card?";
                URL url = new URL(postReceiverUrl);
                Map<String, Object> params = new LinkedHashMap<>();

                params.put("token", token_str);
                StringBuilder postData = new StringBuilder();
                for (Map.Entry<String, Object> param : params.entrySet()) {
                    if (postData.length() != 0) postData.append('&');
                    postData.append(URLEncoder.encode(param.getKey(), "UTF-8"));
                ...
                writer.close();
                reader.close();
                Log.e("valid_card Card", ">>>>>>>>>>>>" + response);
                return response;
            } catch (UnsupportedEncodingException e1) {
            ...
            if (result == null) {
                ...
                try {
                    ...
                    if (status.equalsIgnoreCase("1")) {
                        ...
                        customer_id = jsonObject1.getString("id");

                        JSONArray jsonArray = jsonObject2.getJSONArray("data");
                        for (int k = 0; k < jsonArray.length(); k++) {
                            JSONObject jsonObject3 = jsonArray.getJSONObject(k);
                            card_id = jsonObject3.getString("id");
                            namecard.setText("" + jsonObject3.getString("name"));
                            edt_cardnumber.setText("" + jsonObject3.getString("last4"));
                            expiry_date.setText("" + jsonObject3.getString("exp_month"));
                            security_code.setText("" + jsonObject3.getString("cvv"));
                            year.setText("" + jsonObject3.getString("exp_year"));
                        }
                    } else {
